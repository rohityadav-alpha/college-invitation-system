  ...
  try {
    ...
    const emailLogs = allRecipients.map((recipient, index) => {
      const logData: any = {
        ...
        recipientType: students.find((s: { id: string }) => s.id === recipient.id) ? 'student' :
                      guests.find((g: { id: string }) => g.id === recipient.id) ? 'guest' : 'professor',
        status: 'sent'
      }
      
      if (students.find((s: { id: string }) => s.id === recipient.id)) {
        logData.studentId = recipient.id
      } else if (guests.find((g: { id: string }) => g.id === recipient.id)) {
